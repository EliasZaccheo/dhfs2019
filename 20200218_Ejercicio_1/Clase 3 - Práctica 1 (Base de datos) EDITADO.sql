...
FROM movies m LEFT JOIN genres g ON (g.id=m.genre_id)
ORDER BY g.name DESC;

SELECT m.title, g.name, a.first_name, a.last_name
FROM movies m LEFT JOIN genres g ON (g.id=m.genre_id) LEFT JOIN actor_movie am ON (m.id=am.movie_id) LEFT JOIN actors a ON (a.id=am.actor_id)
ORDER BY g.name DESC;

SELECT a.first_name, m.title
FROM actors a LEFT JOIN actor_movie am ON (a.id=am.actor_id) LEFT JOIN movies m ON (m.id=am.movie_id)
...
UNION
SELECT m.title, g.name
FROM movies m RIGHT JOIN genres g ON (g.id=m.genre_id);
...
SELECT e.title AS capitulo, ss.number AS numero_temporada, s.title AS serie, g.name AS genero, COUNT(*) AS cantidad_actores
FROM episodes e LEFT JOIN seasons ss ON (ss.id=e.season_id) LEFT JOIN series s ON (s.id=ss.serie_id) LEFT JOIN genres g ON (s.genre_id=g.id) LEFT JOIN actor_episode ae ON (ae.episode_id=e.id)
GROUP BY e.id;

SELECT g.name AS genero, AVG (m.rating) AS rating_promedio
FROM genres g LEFT JOIN movies m ON (m.genre_id=g.id)
GROUP BY g.name HAVING rating_promedio > 5;
...
SELECT s.title as serie, COUNT(*) AS cantidad_episodios
FROM series s LEFT JOIN seasons ss ON (ss.serie_id=s.id) LEFT JOIN episodes e ON (e.season_id=ss.id)
WHERE e.release_date BETWEEN '2016-01-01 00:00:00' AND '2017-01-01 00:00:00'
GROUP BY s.title;

SELECT s.title as serie, COUNT(*) AS cantidad_episodios
FROM series s LEFT JOIN seasons ss ON (ss.serie_id=s.id) LEFT JOIN episodes e ON (e.season_id=ss.id)
WHERE e.release_date > '2020-01-01 00:00:00'
GROUP BY s.title;